...
lines, all of it a record of chasing `reference/` photographs, and that comparison is now CLOSED:
the visual bar is met, and `reference/` plus every pixel metric is a REGRESSION GATE rather than a
target. None of what follows is actionable. It is kept only so a claim can be audited by `grep`.

Before the move, every live constraint in it was checked against `tools/STANDING-CONSTRAINTS.md`,
which carries all of it with a verdict-file citation for each claim; the round-by-round detail lives
in `verdicts/wave-p/`, `verdicts/wave-q/` and `verdicts/wave-r/`. The permanent rules and the
concurrency rule were NOT moved: they are process, not visual targets, and they stayed in STATE.md.

Contents: wave P batch 1/2/3 results and the sixteen broken tools, metrics and reference anchors
they found; the wave Q critic sweep (9 of 9) and its anchor retirements; wave R batch 1 (4/4 plus
the resolver) and the four re-registered dusk anchors; the session-14 harness and git findings.
