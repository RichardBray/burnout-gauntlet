# WAVE P BUILDER BRIEF (shared preamble — read this whole file first)
...
1. Your verdict file: `verdicts/wave-o/<your-piece>.md`. **This is your brief.** It names the
...
2. `STATE.md` — read ALL of it. Session 11 split the history out to `STATE-HISTORY.md`, so
   STATE.md is now ~350 lines and every line in it is live. **Never open `STATE-HISTORY.md`**;
   it is 3800 lines of reverse-chronological history, nothing in it is actionable, and it will
   eat your context for nothing. Then `tools/STANDING-CONSTRAINTS.md` for the standing
   constraints and the cross-cutting findings.
...
## CRASH-SAFETY — THIS IS NOT OPTIONAL

**Write your full report to `verdicts/wave-p/<your-piece>.md` BEFORE you return it as your final
message.** Session 8 died between the builders finishing their edits and the orchestrator
collecting their reports, and the ENTIRE Wave L report set was lost — ten builders' worth of
paired A/B numbers, gone, while the code sat in the tree unexplained. The file on disk is the
record. Your final message is only a convenience copy.

## RULE 5 — DO NOT TRUST A DOCSTRING IN THIS TREE. VERIFY THE CONSTANT.

The Wave L crash.js builder rewrote its comments and changed ZERO constants. `crash.js:2118-2127`
claims "peak r sits just above 1.0"; `:2131` still reads `r = 2.8`. `:196-209` documents a `VC`
nose split that does not exist. An intermediate screenshot proves the builder had the fix working
and reverted it before saving. That cost a full critic round to detect.

So, binding on you:
- Any comment in your owner file is UNVERIFIED. Grep the constant before you believe it. Your
  brief's line numbers may point at prose that lies about the code beneath it.
- **Your report MUST quote the BEFORE and AFTER literal value of every constant you changed,
  each with `file:line`.** Not "reduced the gain" — `crash.js:2131  r = 2.8 -> 1.10`.
- If you update a comment, the constant it describes must actually have that value when you save.
- Re-run `./tools/lint.sh` and re-grep your own constants as the LAST thing you do, after your
  final screenshot. The revert above happened somewhere between the working render and the save.

## WAVE P: YOU ARE IN A BATCH, NOT A FREE-FOR-ALL

Wave N ran nine builders at once and paired A/B nearly broke down: car.js went transiently
NON-PARSING inside another builder's measurement window, road.js rendered a false-colour debug
ramp that zeroed every fill statistic, scene exposure moved 73 -> 213 -> 72 mid-round, and
`intactFlank` swung 99.5 -> 194.7 with identical damage.js. Three builders had to recover.

**The recovery technique WORKS and is now standard — use it, do not rediscover it:**
reconstruct your pre-edit file byte-exactly, `md5`-verify it against the original, then render
**A,B,A,B interleaved** in short windows and require both rounds to agree. Report both rounds.
If a peer file's md5 moves between your A and your B, that pair is void — discard and redo.

## EIGHT BROKEN METRICS HAVE BEEN FOUND. Audit before you optimise.

Retired or corrected so far: `_facademeas` band sat (measures blue airlight, not paint);
`_hudlick` baseBot sampling off-frame; `_hudedge` widths in %-of-frame (normalise to barH);
`_debrismeas` aspMed AND aspP90 (aspP90 inverts under boost live — use blob density);
`_ignmeas --lu` mono downmix (~1 LU understated — measure channel-split); `_paintmeas`
grainRMSpct (fixed 3x3 box high-pass, screen-locked, and the ref anchor regions contain no
flake at all); the 1e-4 audio onset gate on mp3 refs (use `IGN_REL=0.1`); a null test that
substitutes a term's CEILING instead of identity.

**And the project's own anti-aliasing guard is not complete.** Scale-persistence P rejects only
PER-PIXEL aliasing; a 20 px coherent comb scores 1.35 and passes. **Pair P with an fx/nofx ratio
— a blur must REDUCE high-frequency energy — and with a crop you actually look at.**

**Budget one tool audit per round.** Tool audits have outperformed pixel work every wave since K.
