// _hr2v-live.mjs — the VERIFY pass's LIVE probe for the one thing the repair's headless numbers
// cannot settle: the trail-brake dead zone the new instant drift entry creates.
//
...
for (const kmh of [100, 130, 200]) {
  for (const [label, extra] of [['W+D held      ', []], ['W+D then S+D  ', ['KeyS']]]) {
    for (const win of [400, 800]) {
      ...
      const swept = Math.abs(ang(end.yaw - win0.yaw)) * DEG;
      const pkYaw = inWin.reduce((m, x) => Math.max(m, Math.abs(x.yawRate)), 0) * DEG;
      const pkSlip = inWin.reduce((m, x) => Math.max(m, Math.abs(x.slipAngle)), 0) * DEG;
...
for (const kmh of [100, 130, 200]) {
  for (const [label, keys] of [['W+D    ', ['KeyW', 'KeyD']], ['S+D    ', ['KeyS', 'KeyD']]]) {
    for (const win of [400, 800]) {
      ...
      const swept = Math.abs(ang(end.yaw - a.yaw)) * DEG;
      const pkYaw = s.reduce((m, x) => Math.max(m, Math.abs(x.yawRate)), 0) * DEG;
...
await writeFile(resolve(root, '..', 'verdicts', 'wave-s', 'handling-r2-verify-live.txt'), out.join('\n') + '\n');
