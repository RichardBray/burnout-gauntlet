...
export function createWorld(scene, { rng, roadKit }) {
  ...
  globalThis.__CRITIC = { drops: 0, dropPools: {}, pools: [], guardDrops: 0 };
  function inst(geo, mat, cap, { cast = true, recv = true } = {}) {
    ...
    m.count = 0;
    m.userData.cap = cap;
    m.userData.tag = 'inst#' + globalThis.__CRITIC.pools.length + '@' + (new Error().stack.split('\n')[2] || '').trim().slice(-40);
    globalThis.__CRITIC.pools.push(m);
    group.add(m);
    return m;
  }
  function push(m, x, y, z, ry = 0, rz = 0, sx = 1, sy = 1, sz = 1, color) {
    if (m.count >= m.userData.cap) {
      const C = globalThis.__CRITIC; C.drops++;
      const t = m.userData.tag || m.name || 'anon';
      C.dropPools[t] = (C.dropPools[t] || 0) + 1;
      return;
    }
  ...
  function streetLight(x, z, rotY) {
    ...
    const used = [];
    const rec = (m) => { if (m.count < m.userData.cap) used.push([m, m.count]); else globalThis.__CRITIC.guardDrops++; };
    rec(slPole);
    ...
    dummy.updateMatrix();
    if (slBulb.count >= slBulb.userData.cap) globalThis.__CRITIC.guardDrops++;
  ...
  function parkedCar(x, z, ry) {
    ...
    for (const dx of [-1.42, 1.46]) {
      for (const dz of [-0.86, 0.86]) {
        ...
        dummy.updateMatrix();
        if (carWheel.count >= carWheel.userData.cap) globalThis.__CRITIC.guardDrops++;
  ...
  globalThis.__CRITIC.snapshot = () => globalThis.__CRITIC.pools.map((m) => ({
    tag: m.userData.tag, count: m.count, cap: m.userData.cap, full: m.count >= m.userData.cap }));
  globalThis.__CRITIC.preCut = globalThis.__CRITIC.snapshot();
  globalThis.__CRITIC.otherIM = (() => { const o = []; group.traverse((x) => {
    if (x.isInstancedMesh && !globalThis.__CRITIC.pools.includes(x))
      o.push({ name: x.name, count: x.count, cap: x.userData.cap }); }); return o; })();
  let chunkStats = null;
