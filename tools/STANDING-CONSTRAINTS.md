...
**Still uncorrected in the tree:** `crash.js:2130-2148` cites this dead anchor as the justification for `s.streak 0.045 -> 0.012`, and `_stripemeas.mjs:11-16` still advertises its own retired anchors as live. Fix both prose blocks in place next time you open those files.
...
**AMENDMENT, WAVE R. THE HISTORICAL `sat` FIGURES ARE AUDITED. THE DIVERGENCE TRIGGER IS HUE DIVERSITY, NOT SMOOTHNESS - AND THE SAME DEFECT LIVES IN A SECOND TOOL** (`verdicts/wave-r/resolver.md` §3).
...
- **CONFLICT RESOLVED, WAVE R: the preserved line "`daytime-downtown-04` is the only reference with our camera" is FALSE AND STRUCK. `-04` IS A DUSK / BLUE-HOUR FRAME AND ANCHORS NOTHING PHOTOMETRIC IN A DAY SCENE** (`verdicts/wave-r/resolver.md` §2, crops re-read independently; `wave-q/environment.md` §1 and §9 UPHELD; `reference/INDEX.md` line corrected in place).
...
**WAVE R RESOLVER PASS. Items 2, 3, 4, 8 and 9 are RESOLVED and are no longer open; their rulings are recorded below in place and in full in `verdicts/wave-r/resolver.md`. Items 1, 5, 6 and 7 are CONFIRMED STILL CORRECTLY PARKED with their live owners and were deliberately not touched. Two new items, 10 and 11, were opened by the resolver's own evidence.**
...
8. **`camera.js`, `main.js`, `physics.js`, `scenes.js`, `util.js` carry no wave-P hash in any verdict.** All five are plausibly untouched but nobody has asserted it (`wave-q/boost-fx.md`).
9. **Prose in the tree still citing retired anchors: `crash.js:2130-2148` and `_stripemeas.mjs:11-16`.** Flagged twice; still unfixed. The next agent to open either file will read a lie.
